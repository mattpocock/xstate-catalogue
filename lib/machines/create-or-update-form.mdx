...
- If the edit or create go wrong, via <Event>error.platform.editItem</Event> or <Event>error.platform.createItem</Event>, we head back to <State>awaitingSubmit</State> with the <Context>errorMessage</Context> saved.

- If the edit goes well, via <Event>done.invoke.editItem</Event> or <Event>done.invoke.createItem</Event>, we're done! We head to the <State>complete</State> state and either <Action>onCreateSuccess</Action> or <Action>onEditSuccess</Action> are fired.
