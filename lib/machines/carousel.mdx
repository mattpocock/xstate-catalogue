...
From this point we can do three things:
 ...
 1. raise the <Event>PAUSE</Event> event to pause the carousel.
 2. raise the <Event>RESET</Event> event to reset back to active index 0, transitioning to the <State>idle</State> state.
 3. raise the <Event>UPDATE_INDEX</Event> event with an index to move to a custom slide index (useful for paginated carousels) but
 remaining in the <State>playing</State> state.

From the <State>paused</State> state we can resume <State>playing</State> from the index we paused at using the <Event>AUTO_PLAY</Event> event, use 
the <Event>RESET</Event> event to reset to active index 0 and transition into the <State>idle</State> state 
or raise the <Event>UPDATE_INDEX</Event> event with a custom index to remain <State>paused</State> which could allow the carousel 
