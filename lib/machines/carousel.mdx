...
From this point we can do three things:
 ...
 3. send the <Event>UPDATE_INDEX</Event> event with an index to move to a custom slide index (useful for paginated carousels) but
...
or send the <Event>UPDATE_INDEX</Event> event with a custom index to remain <State>paused</State> which could allow the carousel 
